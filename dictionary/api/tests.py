...
class IsOwnerPermissionTestCase(APITestCase):
    ...
    def test_has_object_permission_should_return_true_when_object_is_a_dictionary(self):
        result = self.per.has_object_permission(
            self.request, self.view, self.dictionary
        )
...
class CustomUpdateTestCase(APITestCase):
    ...
    def setUp(self):
        self.validated_data = {"title": "Polish", "words": {"love": "miłość"}}

        self.cust_up_obj = CustomUpdate()

    def test_update_should_return_passed_subject_with_values_retrieved_form_validated_data(
        self,
    ):
    ...
    def test_update_should_return_passed_dictionary_with_values_retrieved_form_validated_data(
        self,
    ):
...
class SubjectViewSetTestCase(APITestCase):
    ...
    def test_http_get_method_should_return_status_200_when_list_of_subjects_requested(
        self,
    ):
    ...
    def test_http_post_method_should_return_status_200_when_list_of_subjects_requested(
        self,
    ):
        response = self.client.get(reverse("subject-list"))
    ...
    def test_http_post_method_should_return_status_201_when_requested_with_valid_data(
        self,
    ):
        response = self.client.post(reverse("subject-list"), data={"title": "Polish"})

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_http_post_method_should_increase_amount_of_subjects_when_requested_with_valid_data(
        self,
    ):
        dictionaries_before = Subject.objects.count()

        self.client.post(reverse("subject-list"), data={"title": "Polish"})

        dictionaries_after = Subject.objects.count()

        self.assertGreater(dictionaries_after, dictionaries_before)

    def test_http_post_method_should_return_status_400_when_requested_with_data_of_existed_subject(
        self,
    ):
        response = self.client.post(reverse("subject-list"), data={"title": "English"})
...
class DictionaryViewSetTestCase(APITestCase):
    ...
    def test_http_get_method_should_return_status_200_when_list_of_dictionaries_requested_with_valid_args(
        self,
    ):
    ...
    def test_http_get_method_should_return_status_404_when_requested_with_invalid_args(
        self,
    ):
        response = self.client.get(
            reverse("dictionary-list", args=[self.subject.id + 1])
        )
    ...
    def test_http_post_method_should_return_status_201_when_requested_with_valid_data(
        ...
        response = self.client.post(
            reverse("dictionary-list", args=[self.subject.id]),
            data={"title": "Popular words"},
        )

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_http_post_method_should_increase_amount_of_dictionaries_when_requested_with_valid_data(
        self,
    ):
    ...
    def test_http_post_method_should_return_status_400_when_requested_with_data_of_existed_dictionary(
        self,
    ):
        response = self.client.post(
            reverse("dictionary-list", args=[self.subject.id]),
...
class WordsRequestsTestCase(APITestCase):
    ...
    def test_http_get_method_should_return_status_200_when_list_of_words_from_dictionary_requested_with_valid_args(
        self,
    ):
        response = self.client.get(
            reverse("dictionary-words", args=[self.subject.id, self.dictionary.id])
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_http_get_method_should_return_status_200_when_list_of_words_from_session_requested_with_valid_args(
        self,
    ):
        response = self.client.get(
            reverse("dictionary-edit-words", args=[self.subject.id, self.dictionary.id])
        )
    ...
    def test_http_post_method_should_return_status_200(self):
        response = self.client.post(
            reverse(
                "dictionary-edit-words", args=[self.subject.id, self.dictionary.id]
            ),
    ...
    def test_http_post_method_should_invoke_save_to_db_method_of_words_class(
        self, save_to_db_mock
    ):
        self.client.post(
            reverse(
                "dictionary-edit-words", args=[self.subject.id, self.dictionary.id]
    ...
    def test_http_put_method_should_return_status_400_when_definition_in_provided_data_already_exists_in_words(
        self,
    ):
        response = self.client.put(
            reverse(
    ...
    def test_http_put_method_should_invoke_add_word_method_of_words_class(
        self, add_word_mock
    ):
        self.client.put(
            reverse(
    ...
    def test_http_delete_method_should_invoke_remove_word_method_of_words_class(
        self, remove_word_mock
    ):
        self.client.delete(
            reverse(
                "dictionary-edit-words", args=[self.subject.id, self.dictionary.id]
    ...
    def test_http_delete_method_should_return_status_400_when_provided_definition_does_not_exist_in_words(
        self,
    ):
        response = self.client.delete(
            reverse(
    ...
    def test_http_post_method_should_invoke_refresh_list_method_of_words_class(
        self, refresh_list_mock
    ):
        self.client.post(
            reverse(
                "dictionary-refresh-words", args=[self.subject.id, self.dictionary.id]
            )
        )
