...
class SubjectCreateViewTestCase(TestCase):
    ...
    def test_http_post_method_response_should_return_status_302_when_valid_data_provided(self):
        response = self.client.post(reverse('dictionary:subject_create'), data={"title": "Polish"})
    ...
    def test_http_post_method_response_should_return_status_200_when_attempt_to_create_existed_subject(self):
        response = self.client.post(reverse('dictionary:subject_create'), data={"title": self.subject.title})
...
class SubjectUpdateViewTestCase(TransactionTestCase):
    def setUp(self):
        ...
        self.subject = Subject.objects.create(title="English",
                                              owner=self.user)

        self.another_subject = Subject.objects.create(title="Polish",
                                                      owner=self.user)

        self.client.login(username='TestUser', password='test1234')

    def test_http_get_method_response_should_return_status_200(self):
        response = self.client.get(reverse('dictionary:subject_update', args=[self.subject.slug]))
    ...
    def test_http_post_method_response_should_return_status_302_when_valid_data_provided(self):
        ...
        response = self.client.post(reverse('dictionary:subject_update', args=[self.subject.slug]),
                                    data={'title': new_title})

        self.assertEqual(response.status_code, 302)

    def test_http_post_method_response_should_return_status_200_when_attempt_to_update_data_to_data_of_existed_subject(self):
        response = self.client.post(reverse('dictionary:subject_update', args=[self.subject.slug]),
                                    data={'title': self.another_subject.title})

        self.assertEqual(response.status_code, 200)

    def test_http_post_method_when_attempt_to_update_data_to_data_of_existed_subject_response_should_contains_original_title(self):
        response = self.client.post(reverse('dictionary:subject_update', args=[self.subject.slug]),
                                    data={'title': self.another_subject.title})
...
class SubjectDeleteViewTestCase(TestCase):
    ...
    def test_http_get_method_response_should_return_status_200(self):
        response = self.client.get(reverse('dictionary:subject_delete', args=[self.subject.id]))
    ...
    def test_http_post_method_response_should_return_status_302(self):
        response = self.client.post(reverse('dictionary:subject_delete', args=[self.subject.id]))
...
class DictionaryCreateViewTestCase(TransactionTestCase):
    def setUp(self):
        ...
        self.dictionary = Dictionary.objects.create(title="Basic words",
    ...
    def test_http_post_method_response_should_increase_amount_of_dictionaries(self):
        ...
        self.client.post(reverse('dictionary:dict_create', args=[self.subject.slug]),
                         data={"title": "Popular words"})
    ...
    def test_http_post_method_response_should_return_status_200_when_attempt_to_create_existed_dictionary(self):
        response = self.client.post(reverse('dictionary:dict_create', args=[self.subject.slug]),
...
class DictionaryUpdateViewTestCase(TransactionTestCase):
    def setUp(self):
        ...
        self.another_dictionary = Dictionary.objects.create(title="Popular words",
                                                            subject=self.subject)

        self.client.login(username='TestUser', password='test1234')

    def test_http_get_method_response_should_return_status_200(self):
        response = self.client.get(reverse('dictionary:dict_update', args=[self.subject.slug, self.dictionary.slug]))
    ...
    def test_http_post_method_response_should_return_status_302_when_valid_data_provided(self):
        ...
        response = self.client.post(reverse('dictionary:dict_update', args=[self.subject.slug, self.dictionary.slug]),
                                    data={'title': new_title})

        self.assertEqual(response.status_code, 302)

    def test_http_post_method_response_should_return_status_200_when_attempt_to_update_data_to_data_of_existed_dictionary(self):
        response = self.client.post(reverse('dictionary:dict_update', args=[self.subject.slug, self.dictionary.slug]),
                                    data={'title': self.another_dictionary.title})

        self.assertEqual(response.status_code, 200)

    def test_http_post_method_when_attempt_to_update_data_to_data_of_existed_dictionary_response_should_contains_original_title(self):
        response = self.client.post(reverse('dictionary:dict_update', args=[self.subject.slug, self.dictionary.slug]),
                                    data={'title': self.another_dictionary.title})
...
class DictionaryDeleteViewTestCase(TransactionTestCase):
    ...
    def test_http_get_method_response_should_return_status_200(self):
        response = self.client.get(reverse('dictionary:dict_delete', args=[self.dictionary.id]))
    ...
    def test_http_post_method_response_should_return_status_302(self):
        response = self.client.post(reverse('dictionary:dict_delete', args=[self.dictionary.id]))
...
class WordFormViewTestCase(TestCase):
    ...
    def test_http_get_method_response_should_return_status_200(self):
        response = self.client.get(reverse('dictionary:word_form',
                                           args=[self.subject.slug, self.dictionary.slug]))
    ...
    def test_http_post_method_when_invalid_data_provided_response_should_return_status_200(self):
        response = self.client.post(reverse('dictionary:word_form',
                                            args=[self.subject.slug, self.dictionary.slug]),
    ...
    def test_http_post_method_when_no_data_provided_response_should_return_status_200(self):
        response = self.client.post(reverse('dictionary:word_form',
                                            args=[self.subject.slug, self.dictionary.slug]),
